...
namespace Klocman.Tools
{
    public static class RegistryTools
    {
        ...
        /// <summary>
        /// Fully qualified path to regedit.exe. Never launch it by bare name - this process runs
        /// elevated with its working directory set to the application directory, which is
        /// user-writable for portable installs.
        /// </summary>
        private static string RegeditPath { get; } = Path.Combine(
        ...
        private static void RunRegeditCommand(string command)
        {
            ...
            var startInfo = new ProcessStartInfo(RegeditPath, command)
        ...
        private static string[] ExportRegistryHelper(string registryPath)
        {
            ...
            var tempDirectory = Directory.CreateDirectory(Path.Combine(temp, "MOKSH"));
