...
namespace BulkCrapUninstaller.Functions.Ratings
{
    public partial class UninstallerRatingManager
    {
        ...
        {
            // No backend configured - ratings stay local. Note the upstream response path decoded
            // Base64 -> Brotli -> JSON from a cleartext-HTTP source inside an elevated process with no
            // size cap; if a backend is ever added, bound the decompressed size before deserializing.
            if (!Program.HomeServerAvailable) return;
