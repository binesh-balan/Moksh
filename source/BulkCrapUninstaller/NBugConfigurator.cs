...
namespace BulkCrapUninstaller
{
    ...
    public static class NBugConfigurator
    {
        public static void SetupNBug()
        {
            ...
            // No destination is registered unless a backend is configured, so crash details are shown
            // to the user and written to the local log but never leave the machine. Upstream sent them
            // unconditionally - the send was gated only by NBug's own dialog, not by any privacy
            // setting - over cleartext HTTP to a third-party host. See Program.ConnectionString.
            if (Program.HomeServerAvailable)
