...
namespace BulkCrapUninstaller
{
    internal static class EntryPoint
    {
        ...
        private const string MUTEX_NAME = @"Global\MOKSH-singleinstance";
        ...
        private static void HandleBeingSecondInstance()
        {
            try
            {
                ...
                {
                    CustomMessageBox.ShowDialog(null, new CmbBasicSettings("MØKSH is already running", "MØKSH is already running", "You can start only one instance of MØKSH. Close previous instances and try again. If you don't see the MØKSH window or it's not responding, try closing it with Task Manager.", DrawingTools.ExtractAssociatedIcon(location), "OK"));
                }
            }
            catch (Exception ex)
            {
