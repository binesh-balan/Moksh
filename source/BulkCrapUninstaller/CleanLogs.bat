title Removing MOKSH log files...
@echo off
cls
...
REM Wait for MOKSH to exit so the logs are written
echo Waiting for MOKSH to exit...
:loop
tasklist | find "Moksh.exe " >nul
if not errorlevel 1 (
...
del /f /s Moksh.exe.log SteamHelper.exe.log StoreAppHelper.exe.log UninstallerAutomatizer.exe.log UpdateHelper.exe.log

echo Deleting .NET generated drectories...
REM Settings directory automatically created by .NET
set SETTINGSDIR=%APPDATASAFE%\Binesh_Balan

if not exist %SETTINGSDIR% (
exit
)

REM Remove all directories related to MOKSH
for /d %%G in ("%SETTINGSDIR%\Moksh*") do rd /s /q "%%~G"
