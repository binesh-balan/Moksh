...
namespace BulkCrapUninstaller.Forms
{
    internal sealed partial class MainWindow : Form
    {
        ...
        /// <summary>
        /// Take ownership of a directory and grant Administrators full control.
        /// </summary>
        /// <remarks>
        /// directoryPath originates from a registry InstallLocation/UninstallerLocation value, which
        /// any standard user can write. It is never passed through a shell: both tools are invoked
        /// directly with ArgumentList, so quotes and the cmd.exe operators (&amp; | &lt; &gt; ^) are
        /// passed as literal argument text instead of being parsed as commands.
        /// </remarks>
        private static void TakeOwnership(string directoryPath)
        {
            if (string.IsNullOrWhiteSpace(directoryPath) || !Directory.Exists(directoryPath))
            {
                PremadeDialogs.GenericError($"The directory does not exist:\n{directoryPath}");
                return;
            }

            var systemDir = WindowsTools.GetEnvironmentPath(CSIDL.CSIDL_SYSTEM);

            try
            {
                var output = new StringBuilder();

                if (!RunOwnershipTool(Path.Combine(systemDir, "takeown.exe"),
                        new[] { "/f", directoryPath, "/r", "/d", "y" }, output))
                    return;

                if (!RunOwnershipTool(Path.Combine(systemDir, "icacls.exe"),
                        new[] { directoryPath, "/grant", "administrators:F", "/t", "/c" }, output))
                    return;

                MessageBox.Show(output.ToString().Trim(), "Take ownership",
        ...
        private static bool RunOwnershipTool(string executable, string[] arguments, StringBuilder output)
        {
            ...
            using (var process = Process.Start(psi))
            {
                if (process == null)
                {
                    PremadeDialogs.GenericError($"Failed to start {Path.GetFileName(executable)}");
                    return false;
                }
                ...
                if (process.HasExited && process.ExitCode != 0)
                {
                    PremadeDialogs.GenericError(
                        $"{Path.GetFileName(executable)} returned error code {process.ExitCode}\n\n{error}");
                    return false;
                }
            }
        ...
        {
            // Failures here used to be silent and load-bearing: this runs from the list-refresh
            // completion handler, so an exception building the wizard aborted the rest of that
            // handler and left the loading overlay on screen forever, with nothing in the log.
            try
            {
            ...
            {
                Console.WriteLine("Failed to show the setup wizard: " + ex);
                PremadeDialogs.GenericError(ex);
            }
        }
                ...
                if (d.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        File.WriteAllLines(d.FileName,
                            new[] { "@echo off", "echo MOKSH generated batch uninstall script" }.Concat(
                            _listView.SelectedUninstallers
