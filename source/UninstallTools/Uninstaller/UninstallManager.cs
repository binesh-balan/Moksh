...
namespace UninstallTools.Uninstaller
{
    public static class UninstallManager
    {
        ...
        private static void UpdateNsisStartInfo(ProcessStartInfo startInfo, string entryName)
        {
            ...
            // Copy into a fresh, randomly named directory rather than straight into %TEMP% under a
            // predictable name. The old name was derivable from the display name, File.Copy overwrote
            // unconditionally, and the copy was then executed elevated - so where %TEMP% is shared
            // (a SYSTEM-context run resolves it to C:\Windows\Temp) a local user could pre-place a
            // hardlink to gain an arbitrary overwrite, or win the copy-then-start race.
            var tempDir = Directory.CreateDirectory(
                Path.Combine(Path.GetTempPath(), "BCU_" + Path.GetRandomFileName()));

            var tempPath = Path.Combine(tempDir.FullName, newName);

            // CreateNew rather than Copy(overwrite: true) - fail rather than follow a planted link.
            using (var source = new FileStream(startInfo.FileName, FileMode.Open, FileAccess.Read, FileShare.Read))
