...
namespace UninstallTools.Junk.Finders
{
    public abstract class JunkCreatorBase : IJunkCreator
    {
        ...
        {
            // Path-boundary aware: a bare StartsWith treats "C:\Program Files\App" as a prefix of
            // "C:\Program Files\AppData", which both misses directories that are in use and flags ones
            // that are not.
            return !string.IsNullOrEmpty(location)
                   && otherInstallLocations.Any(x => PathTools.SubPathIsInsideBasePath(location, x, true, true));
        }
        ...
        protected FileSystemJunk GetJunkNodeFromLocation(IEnumerable<string> otherInstallLocations, string directory, ApplicationUninstallerEntry app)
        {
            try
            {
                ...
                // OrdinalIgnoreCase: string.Contains(string) is case-sensitive, so a registry value
                // reading "c:\windows\..." slipped past a guard built from "C:\Windows".
                if (dirInfo.FullName.Contains(FullWindowsDirectoryName, StringComparison.OrdinalIgnoreCase)
                    || PathTools.IsProtectedSystemPath(dirInfo.FullName)
                    || !dirInfo.Exists
                    || dirInfo.Parent == null)
                    return null;
