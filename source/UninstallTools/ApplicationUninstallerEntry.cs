    ...
    public class ApplicationUninstallerEntry
    {
        ...
        public X509Certificate2 GetCertificate()
        {
            if (!_certificateGotten)
            {
                ...
                _certificate = CertificateGetter.TryGetCertificate(this, out var sourceFile);

                if (_certificate != null)
                {
                    // Verify the signature over the file, not just the certificate chain. A cert that
                    // chains to a trusted root proves nothing about the bytes it was lifted from.
                    // Certificates not sourced from a file on disk (MSI store) stay "unknown" rather
                    // than being reported as valid.
                    _certificateValid = sourceFile == null
                        ? null
                        : AuthenticodeTools.IsTrusted(sourceFile);
                }
            }
            return _certificate;
        }
