    ...
    public sealed class ChocolateyFactory : IIndependantUninstallerFactory
    {
        private static bool GetChocoInfo(out string chocoLocation)
        {
            ...
            try
            {
                ...
                // We run elevated, so refuse to execute a discovered binary that a standard user
                // could have replaced.
                if (FilesystemTools.IsWritableByNonAdministrators(chocoPath))
                {
                    Trace.WriteLine($"Ignoring Chocolatey: {chocoPath} is writable by non-administrators");
                    return false;
                }
