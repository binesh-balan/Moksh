        ...
        public void AddMissingInformation(ApplicationUninstallerEntry target)
        {
            ...
            if (!string.IsNullOrEmpty(uninstallString))
            {
                target.UninstallerKind = GetUninstallerType(uninstallString);
                     ...
                     && !PathTools.IsProtectedSystemPath(target.InstallLocation)
                     && !UninstallToolsGlobalConfig.IsSystemDirectory(target.InstallLocation))
            {
                ...
                // InstallLocation comes straight from the registry (including HKCU, which any standard user can
                // write), so it must be screened before it can turn into a recursive delete.
