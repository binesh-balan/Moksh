...
begin
  ...
  // Always download. Upstream skipped the download when a file of the expected name already existed
  // in {tmp} and then ran it elevated - so anywhere {tmp} is shared (a SYSTEM-context install
  // resolves it under C:\Windows\Temp, which is world-writable) a local user could pre-place a
  // binary and have setup execute it. Deleting any pre-existing file also stops a planted file from
  // surviving as the download target.
...
begin
  // https://dotnet.microsoft.com/download/dotnet/8.0
  //
  // TODO(security, BCU-M05): the last Dependency_Add argument before the booleans is the expected
  // SHA-256 of the download. It is empty here, which tells Inno Setup to skip integrity
  // verification entirely - HTTPS to Microsoft is the only control in play. Fill it in with the
  // published hash for the exact runtime build referenced below, taken from
  // https://dotnet.microsoft.com/download/dotnet/8.0 , before shipping an installer.
  // This is the only dependency MOKSH actually enables, so it is the only one that needs a hash.
